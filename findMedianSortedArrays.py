class Solution(object):
    def findMedianSortedArrays(self,nums1,nums2):
            #find median of two sorted arrays nums1 nums2
            #leetCode problem #4 in algorithms
            L1=len(nums1)
            L2=len(nums2)
            L=L1+L2
            out=[]
...
#                    La=min(La,midA+2-(st_a))
            ...
            if Lb>La:
                kPrime=k-st_a-st_b
                stDel=max(0,kPrime-La)  #assuming worse case all element in a are<= b[st_b].   
                Lnew=kPrime+2-stDel
                st_bOld=st_b
                st_b=stDel+st_bOld
...
import numpy as np
A=Solution()
s1=np.random.randint(low=1,high=254000)
s2=np.random.randint(low=1,high=240000)
